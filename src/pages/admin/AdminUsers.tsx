...
const mockUsers = [
  { id: '1', name: 'Alice Smith', email: 'alice@example.com', role: 'user', status: 'active', joined: '2025-01-15' },
  { id: '2', name: 'Bob Jones', email: 'bob@example.com', role: 'admin', status: 'active', joined: '2024-11-20' },
  { id: '3', name: 'Charlie Brown', email: 'charlie@example.com', role: 'user', status: 'suspended', joined: '2026-03-10' },
];
            ...
            <tbody className="divide-y divide-stone-200 dark:divide-stone-800">
              {mockUsers.map((user) => (
                <tr key={user.id} className="hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium capitalize ${user.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'}`}>
                      {user.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 flex justify-end gap-2">
                    ...
                    <button title={user.status === 'active' ? 'Suspend User' : 'Activate User'} className="p-2 text-stone-400 hover:text-amber-500 transition-colors">
